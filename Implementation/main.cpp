...
const std::vector<std::string> kIndexStructures{"ART", "Trie", "CTrie"};
                           ...
                           const Benchmark benchmark, const uint32_t size)
{
    ...
    std::uniform_int_distribution<uint32_t> search_numbers_distr(0, size);
...
auto RunBenchmarkIteration(const Benchmark benchmark, const uint32_t size)
{
    ...
    std::cout << "\nFinished art. Deleting..." << std::endl;
    ...
    clock.Start();
    CTrieInsert(ctrie, numbers, size);
    time = clock.Stop();

    if (benchmark == Benchmark::kSearch)
    {
        clock.Start();
        CTrieSearch(ctrie, search_numbers, size);
        time = clock.Stop();
    } else if (benchmark == Benchmark::kRangeSearch)
    {
    }

    std::cout << "\nFinished ctrie. Deleting..." << std::endl;
